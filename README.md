...
1. **I give it links.** As many as I want, for one entity. Website, YouTube, Instagram, Facebook, LinkedIn, GitHub, Medium, Substack, Reddit, X/Twitter, Linktree/Beacons/stan.store, Patreon, Ko-fi/Buy Me a Coffee, Twitch, or literally any other website (it has a generic fallback collector for that — no site left behind).
...
3. **It optionally adds a news feed for free** — if I gave it an entity name, it auto-attaches a Google News search for that name. I didn't even ask. It just does it. Overachiever.
4. **A local vision model reads every screenshot.** Not to transcribe text I already scraped (that's how you get an AI confidently telling you a YouTuber has "451K views" when it's actually 451 — true story, ask me how I know), but to describe layout, branding, vibe, and to OCR the stuff that genuinely only exists as pixels (login-walled social previews, infographics, thumbnails).
...
6. **I get a document**, not a spreadsheet of raw scrape dumps. Markdown, or click one button for a proper server-rendered PDF with page numbers and everything.

---

## The "wait, why should I trust this" section

Fair question. Here's what I specifically built in to stop this thing from confidently lying to me:

- **Text-first, vision-as-a-backup.** If I already scraped a page's real text, the vision model is told to describe the *design*, not re-read the words — because re-reading them is exactly how numbers get hallucinated. (See: the entire 451-vs-451K incident above. I learned. Painfully.)
...
- **Failures are boring on purpose.** If a source 404s, gets rate-limited, or a platform decides my IP looks like a robot (rude, but fair, it is a robot), the dossier just says *"No notable public information found."* It will never print "HTTP 403" or a stack trace into my nice research document. That stuff lives in the debug view where it belongs.
- **No login automation, anywhere.** Instagram, Facebook, LinkedIn, Reddit, Patreon — all logged-out, best-effort, public-data-only. I'm not trying to get my account banned or my IP blacklisted. Some sources will come back thin. That's the deal with polite scraping, and I think it's the right one.
...
- **Backend:** FastAPI + a single-worker async job queue. Why single-worker? Because my GPU can only hold one model at a time — the vision model and the synthesis model take turns, tag-team style, and swapping two at once would just set my VRAM on fire. SQLite handles the job store, the YouTube quota ledger, and a TTL cache so repeat lookups are free.
...
- **Local AI runtime:** [llama.cpp](https://github.com/ggml-org/llama.cpp)'s `llama-server`, CUDA build, running:
  - **Vision:** `Qwen3.5-9B-Q8_0` — chosen after an actual empirical bake-off against Gemma-4-12B on a real screenshot. Gemma read "451 views" as "451K views" and hallucinated a channel called **"PowerDog"** that does not exist. Qwen got it right. I have the receipts in the commit history. This is not a drill.
  - **Synthesis:** `gpt-oss-20b` at high reasoning effort — because inferring a creator's monetization strategy from three sentences of bio text is, unironically, a reasoning task.
- **Frontend:** Vanilla HTML/CSS/JS. No framework, no build step, no 400MB of `node_modules` judging me from a corner. It just works, and I can read all of it in one sitting.
- **Job IDs are sequential and kind of delightful:** the first job is `ABC0001`, then `ABC0002`, all the way to `ABC9999`, then it rolls over to `ABD0001` like an odometer, and if I somehow run 26³ × 9999 jobs it grows a fourth letter. I'm choosing to believe I will never hit that limit. If you do, open an issue, I have questions.
...
| Platform | How | Notes |
|---|---|---|
| **YouTube** | Official Data API v3 + free transcript fetch + comments | The one platform with a real, sanctioned API. I treat it like royalty. |
...
| **Patreon** | Parses the embedded Next.js JSON blob for real membership tiers + prices | This one took real detective work — Patreon hides tier data behind a JSON:API "sideloading" pattern I had to reverse-engineer against a live page. Worth it: real `$3/mo`, `$7/mo` tier pricing, not vibes. |
| **Ko-fi / Buy Me a Coffee** | og-tag scraping + a soft-404 detector | Fun fact: Ko-fi 200s a fake "page" for creators who don't exist instead of a real 404. I catch that so it doesn't get reported as a real profile. Sneaky, Ko-fi. Sneaky. |
...
| **Linktree / Beacons / stan.store** | Discovers every outbound link on the page | One pasted link-in-bio → a full map of everywhere else that creator lives online. Basically a treasure map. |
...
| **Everything else** | Generic Playwright render → text + screenshot | The "I got you" fallback. |

---

## Setup

You'll need: **Python 3.13**, **an NVIDIA GPU with ~16GB VRAM** (built and tuned against an RTX 5060 Ti — smaller cards may need to swap to lighter models), and patience for one big model download if you don't already have GGUFs sitting around from LM Studio or similar.
...
Download a [llama.cpp release](https://github.com/ggml-org/llama.cpp/releases) build matching your CUDA version, drop `llama-server.exe` + its DLLs into `tools/llamacpp/`. Point `backend/config.py` (or your `.env`) at:
...
GET  /api/jobs/{id}/file?path=...            fetch a screenshot/image (path-traversal safe, I checked)
...
Every response follows `{data, cached, fetched_at, quota}`; every error follows `{error: {reason, message}}` with a real HTTP status. Rate-limited, because I like my quota and assume you like yours too.

---

## Known limitations (being honest here, not humble-bragging)

- **Reddit, Instagram, Facebook, LinkedIn, Patreon, Ko-fi** are all logged-out best-effort. Sometimes they'll give you everything, sometimes they'll give you nothing and a shrug. This is the nature of scraping platforms that actively don't want to be scraped, and I chose "degrade gracefully" over "log in and risk getting banned."
...
- **The Great Screenshot Disappearing Act:** llama.cpp silently drops oversized images instead of erroring. I spent a while wondering why the vision model was "reading" a 7000px-tall screenshot and returning nothing, before realizing the image never actually made it to the model. It just... vanished. Into the void. No apology, no explanation. Fixed by downscaling everything to a sane size first.
- **The AI Almost Leaked Its Inner Monologue Into a Professional Document:** gpt-oss is a reasoning model, and when it runs out of tokens mid-thought, it can return its raw internal reasoning instead of a final answer. My first version had a fallback that would print that reasoning straight into the dossier if the real answer was empty. Which meant, briefly, a research document could open with something like *"Okay, I need to figure out what this creator does..."* instead of, y'know, an actual answer. Extremely unprofessional. Fixed.
...
- **The Gemma vs. Qwen Vision Showdown:** I didn't just pick a vision model and hope. Ran an actual head-to-head bake-off on a real screenshot and one model straight-up invented a YouTuber named "PowerDog." Not making that up. It's in the git log if you don't believe me.

---

## About me

I'm 15, I run on caffeine-adjacent energy drinks and main-character delusion, and I half-jokingly refer to myself as an "AI god" 😅 — mostly because I will absolutely rebuild an entire pipeline at 1am over a vision-model hallucination out of pure spite. This is a solo hobby project, built because I wanted it to exist and nobody else was going to build it for me. If you find bugs, no you didn't. If you find something cool, that one was on purpose.
